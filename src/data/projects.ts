export type Project = {
  slug: string;
  name: string;
  type: string;
  role: string;
  image: string;
  technologies: string[];
  description: string;
  summary: string;
  problem: string;
  solution: string;
  demonstrates: string[];
  visual: "sports" | "business" | "dashboard";
};

export const projects: Project[] = [
  {
    slug: "reservas-deportivas",
    name: "Reservas deportivas",
    type: "Plataforma fullstack",
    role: "Frontend + API + Base de datos",
    image: "/projects/reservas-deportivas.svg",
    technologies: ["Next.js", "FastAPI", "PostgreSQL", "Cloudflare R2"],
    description:
      "Plataforma tipo Airbnb para canchas deportivas: buscas, reservas y pagas en línea. Yo construí la interfaz de reserva con calendario y cupos en tiempo real, la API de disponibilidad y el modelo de datos.",
    summary: "Menos coordinación. Más tiempo en la cancha.",
    problem:
      "Conectar la búsqueda de una cancha con su disponibilidad y reserva, en una misma experiencia.",
    solution:
      "Una plataforma con búsqueda por comuna y horario, calendario de reservas y roles para jugadores, dueños de cancha y administradores.",
    demonstrates: [
      "Búsqueda por comuna y horario",
      "Reserva con calendario y cupos",
      "Roles: jugador, dueño de cancha, admin",
    ],
    visual: "sports",
  },
  {
    slug: "sitio-negocios",
    name: "Sitios para negocios",
    type: "Diseño + Desarrollo web",
    role: "Diseño + Desarrollo completo",
    image: "/projects/sitio-negocios.svg",
    technologies: ["Next.js", "Tailwind CSS", "Vercel"],
    description:
      "Landing pages para negocios locales que necesitaban presencia digital seria. Diseño directo, carga rápida y estructura pensada para que el cliente reciba consultas, no solo visitas.",
    summary: "Una presencia digital a la altura de cada negocio.",
    problem:
      "Negocios locales que necesitan presentar sus servicios con claridad y recibir consultas desde la web.",
    solution:
      "Sitios responsive a medida, con SEO técnico básico y despliegue en Vercel con dominio propio.",
    demonstrates: [
      "Diseño responsive a medida",
      "SEO técnico básico",
      "Deploy en Vercel con dominio propio",
    ],
    visual: "business",
  },
  {
    slug: "panel-administrativo",
    name: "Panel administrativo",
    type: "Dashboard · Producto interno",
    role: "Frontend + Integración API",
    image: "/projects/sistema-administrativo.svg",
    technologies: ["React", "APIs REST", "PostgreSQL"],
    description:
      "Sistema interno para gestionar usuarios, registros y operaciones: tablas con filtros, edición en línea y control de permisos. El foco estaba en que una persona no técnica pudiera operarlo sin capacitación.",
    summary: "La complejidad, detrás. El control, al frente.",
    problem:
      "Gestionar usuarios, registros y operaciones desde una interfaz comprensible para personas no técnicas.",
    solution:
      "Un panel con filtros, edición en línea, control de permisos y métricas operativas en un vistazo.",
    demonstrates: [
      "Tablas con filtros y paginación",
      "Gestión de usuarios y roles",
      "Métricas operativas en un vistazo",
    ],
    visual: "dashboard",
  },
];
