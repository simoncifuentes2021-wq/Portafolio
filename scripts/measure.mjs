...
const metrics = await page.evaluate(() => ({
  ...
  javascriptBytes: performance.getEntriesByType("resource").filter(r => r.name.endsWith(".js")).reduce((sum, r) => sum + r.encodedBodySize, 0),
  fontRequests: performance.getEntriesByType("resource").filter(r => r.name.includes(".woff2")).length,
  imageRequests: performance.getEntriesByType("resource").filter(r => r.initiatorType === "img").length,
}));
const metadata = [];
for (const path of ["/", "/projects", "/contact", "/projects/reservas-deportivas", "/projects/sitio-negocios", "/projects/panel-administrativo"]) {
...
const report = { note: "Local production measurement, no throttling; not field Core Web Vitals.", metrics, errors, metadata };
await fs.writeFile("artifacts/production-report.json", JSON.stringify(report, null, 2));
